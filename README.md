...
CLI tool that fetches recent AI research from multiple sources, categorizes them, and generates Markdown and HTML reports.
...
# Deduplicate against prior reports, but keep items from the report being regenerated
python -m knowledge_producer --days 1 --dedup all
...
**Reports** are saved to both `reports/report-{date}-{days}d.md` and `reports/report-{date}-{days}d.html` (e.g. `report-2026-02-28-7d.md` and `report-2026-02-28-7d.html`).

**Logs** are saved to `logs/{date}-{days}d-{HHMMSS}.log` for each generation run.

When using `--dedup all`, the current target report file is excluded from dedup scanning so regenerating, for example, the `2026-03-02` report does not filter out papers already present in `report-2026-03-02-1d.md`.
...
- An HTML version is generated alongside the Markdown report for easier reading in a browser

Example report header:
```
# AI Research Daily Report - 2026-02-28

> Generated on 2026-03-01 13:52:46 PST | `days=7` `sources=all` `max_results=500` ...
>
